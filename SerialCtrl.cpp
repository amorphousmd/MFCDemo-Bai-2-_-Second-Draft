...
BOOL SerialCtrl::ClosePort(void)
{
	if (m_portStatus == TRUE)               // Port need to be open before.
	{
		...
		{
			AfxMessageBox((LPCTSTR)"Port Closing isn't successes.");
			return FALSE;
		}
		return TRUE;
	}
	...
	while (1)
	{
		...
		while (m_serialIO->GetProcessActivateValue() == TRUE)
		{
			if ((serialCtrl().GetPortStatus() == FALSE) && m_serialIO->GetPortActivateValue() == TRUE)
			{
				...
				if (serialCtrl().OpenPort(m_serialIO->m_DCB, (char*)(LPTSTR)(LPCTSTR)m_serialIO->m_csPortName) == TRUE)
				{
...
CSerialIO::CSerialIO() :m_serialProcess(NULL), m_csPortName("COM1"), m_strBaudRate("115200"), m_sendSize(0)
{
...
BOOL CSerialIO::Init()
{
	...
	char* cBaudConvert = (char*)(LPTSTR)(LPCTSTR)m_strBaudRate;
	switch (atoi(cBaudConvert))
	{
	case 110:
